...
ROOM_STATUSES = ["dostępny", "zajęty", "sprzątanie", "wyłączony z użytkowania"]
...
ROOM_STATUS_MAPPING = {
    ...
    "zajęty": "occupied",
    "sprzątanie": "cleaning",
    "wyłączony z użytkowania": "out of service"
}
...
def display_table(data_list, headers_map, title="Wyniki"):
    if not data_list:
        print(Fore.YELLOW + f"Brak danych do wyświetlenia dla: {title}.")
        logger.info(f"Wyświetlono pustą tabelę dla: {title}.")
        return

    column_widths = {header: len(header) for header in headers_map.keys()}
    for item in data_list:
        for header, attr in headers_map.items():
            value = item.get(attr, "N/A") if isinstance(item, dict) else getattr(item, attr, "N/A")
            if isinstance(value, float):
                temp_value_str = f"{value:.2f}"
                value_str = f"{temp_value_str:<{column_widths[header]}}"
            elif isinstance(value, list):
                value_str = f"{', '.join(map(str, value)):<{column_widths[header]}}"
            else:
                value_str = f"{str(value):<{column_widths[header]}}"
            column_widths[header] = max(column_widths[header], len(value_str))
    
    print(Fore.CYAN + Style.BRIGHT + f"\n--- {title} ---")
    logger.info(f"Wyświetlono tabelę: {title}")
    header_line = " | ".join([f"{header:<{column_widths[header]}}" for header in headers_map.keys()])
    print(Fore.BLUE + Style.BRIGHT + header_line)
    print(Fore.BLUE + "-" * len(header_line))
    ...
    print(Fore.CYAN + Style.BRIGHT + "-" * len(header_line))

def display_menu(current_user_role):
    logger.info(f"Wyświetlono główne menu dla roli: {current_user_role}")
    ...
    if current_user_role == "administrator" or current_user_role == "recepcjonista":
    ...
    elif current_user_role == "recepcjonista":
        print(Fore.YELLOW + "7. Generuj Raporty")
        print(Fore.YELLOW + "8. Wyjdź")
    else:
        print(Fore.YELLOW + "6. Generuj Raporty")
        print(Fore.YELLOW + "7. Wyjdź")
    print(Style.BRIGHT + "-----------------------------------")
    ...
    while True:
        ...
        if choice == '1':
            ...
            if not current_user.has_permission("recepcjonista"):
                ...
                continue
            print(Fore.CYAN + "\n--- Zmień Status Pokoju ---")
            number = input("Wprowadź numer pokoju, aby zmienić status: ")
            new_status_pl = get_numerical_choice(ROOM_STATUSES, "Wybierz nowy status pokoju:")
            if not new_status_pl:
                print(Fore.RED + "Status pokoju jest wymagany. Anulowano zmianę statusu.")
                continue
            new_status_en = ROOM_STATUS_MAPPING.get(new_status_pl)
            if new_status_en:
                room_service.update_room_status(number, new_status_en)
                logger.info(f"Próba zmiany statusu pokoju {number} na {new_status_en}")
            else:
                print(Fore.RED + f"Nieprawidłowy status '{new_status_pl}'. Spróbuj ponownie.")
                logger.warning(f"Użytkownik wprowadził nieprawidłowy status pokoju: {new_status_pl}")
            ...
            print(Fore.CYAN + "\n--- Sortuj Pokoje ---")
            sort_options = ["number", "floor", "room_type", "price", "status"]
            sort_by = get_numerical_choice(sort_options, "Sortuj według:")
            if not sort_by:
...
def manage_guests_menu(guest_service: GuestService, current_user: User):
    ...
    while True:
        ...
        if choice == '1':
            print(Fore.CYAN + "\n--- Dodaj Nowego Gościa ---")
            guest_id = input("Wprowadź ID gościa (np. GUEST001): ")
            ...
            guest_id = input("Wprowadź ID gościa do aktualizacji poziomu lojalności: ")
            guest_service.update_guest_loyalty_tier(guest_id)
            ...
            print(Fore.CYAN + "\n--- Sortuj Gości ---")
            sort_by_options = ["guest_id", "first_name", "last_name", "is_vip", "is_loyal_customer", "loyalty_tier"]
            sort_by_choice = get_numerical_choice(sort_by_options, "Sortuj gości według (np. 1.guest_id, 2.first_name, etc.):")
            if not sort_by_choice: continue

            reverse_sort = input("Odwróć kolejność? (tak/nie): ").lower() == 'tak'
            all_guests = guest_service.list_all_guests()
            sorted_guests = guest_service.sort_guests(all_guests, sort_by_choice, reverse_sort)
...
def manage_reservations_menu(reservation_service: ReservationService, room_service: RoomService, guest_service: GuestService, payment_service: PaymentService, current_user: User):
    ...
    while True:
        ...
        if choice == '1':
            ...
            display_table(all_reservations, {"ID Rezerwacji": "reservation_id", "ID Gościa": "guest_id", "Numer Pokoju": "room_number", "Zameldowanie": "check_in_date", "Wymeldowanie": "check_out_date", "Cena": "total_price", "Status": "status", "Status Płatności": "payment_status", "Rabat": "applied_discount_id"}, "Wszystkie Rezerwacje")
            ...
            payment_status_search = input("Wprowadź status płatności do wyszukania (np. pending, paid, partially_paid, pozostaw puste, aby zachować): ")
            if payment_status_search: search_params['payment_status'] = payment_status_search
            ...
            display_table(found_reservations, {"ID Rezerwacji": "reservation_id", "ID Gościa": "guest_id", "Numer Pokoju": "room_number", "Zameldowanie": "check_in_date", "Status": "status"}, "Znalezione Rezerwacje")

        elif choice == '9':
            print(Fore.CYAN + "\n--- Sortuj Rezerwacje ---")
            sort_by_options = ["reservation_id", "guest_id", "room_number", "check_in_date", "check_out_date", "total_price", "status", "payment_status"]
            sort_by_choice = get_numerical_choice(sort_by_options, "Sortuj rezerwacje według (np. 1.reservation_id, 2.guest_id, etc.):")
            if not sort_by_choice: continue

            reverse_sort = input("Odwróć kolejność? (tak/nie): ").lower() == 'tak'
            all_reservations = reservation_service.list_all_reservations()
            sorted_reservations = reservation_service.sort_reservations(all_reservations, sort_by_choice, reverse_sort)
            display_table(sorted_reservations, {"ID Rezerwacji": "reservation_id", "ID Gościa": "guest_id", "Numer Pokoju": "room_number", "Zameldowanie": "check_in_date", "Status": "status"}, f"Posortowane Rezerwacje według {sort_by_choice}")
...
def manage_payments_menu(payment_service: PaymentService, current_user: User):
    ...
    while True:
        ...
        if choice == '1':
            ...
            display_table(all_invoices, {"ID Faktury": "invoice_id", "ID Rezerwacji": "reservation_id", "ID Gościa": "guest_id", "Data Wystawienia": "issue_date", "Termin Płatności": "due_date", "Suma": "total_amount", "Zapłacono": "paid_amount", "Status": "status"}, "Wszystkie Faktury")

        elif choice == '5':
            outstanding_invoices = payment_service.get_outstanding_invoices()
            display_table(outstanding_invoices, {"ID Faktury": "invoice_id", "ID Rezerwacji": "reservation_id", "Suma": "total_amount", "Zapłacono": "paid_amount", "Status": "status"}, "Nieuregulowane Faktury")
...
def manage_housekeeping_menu(housekeeping_service: HousekeepingService, room_service: RoomService, current_user: User):
    ...
    while True:
        ...
        if choice == '1':
            ...
            task_id = input("Wprowadź ID zadania do zmiany statusu: ")
            new_status_pl = get_numerical_choice(HOUSEKEEPING_TASK_STATUSES, "Wybierz nowy status:")
            if not new_status_pl:
            ...
            display_table(all_tasks, {"ID Zadania": "task_id", "Numer Pokoju": "room_number", "Przypisane do": "assigned_to", "Termin": "due_date", "Status": "status", "Notatki": "notes"}, "Wszystkie Zadania Sprzątania")
            ...
            display_table(daily_schedule, {"ID Zadania": "task_id", "Numer Pokoju": "room_number", "Przypisane do": "assigned_to", "Termin": "due_date", "Status": "status", "Notatki": "notes"}, f"Harmonogram Sprzątania na {date_str}")
...
def get_numerical_choice(options: list, prompt: str, allow_empty: bool = False):
    while True:
        print(Fore.YELLOW + prompt)
        for i, option in enumerate(options):
            print(f"{i + 1}. {option}")
        if allow_empty:
            print(f"{len(options) + 1}. Pomiń / Wszystkie / Nie dotyczy")

        choice = input(Fore.GREEN + "Wprowadź numer wyboru: ")
        if allow_empty and not choice.strip(): 
            return None

        if allow_empty and choice == str(len(options) + 1):
            return None
        try:
            index = int(choice) - 1
            if 0 <= index < len(options):
                ...
                print(Fore.RED + "Nieprawidłowy numer. Spróbuj ponownie.")
        except ValueError:
            print(Fore.RED + "Nieprawidłowy wybór. Wprowadź numer.")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if current_user.role == "administrator" or current_user.role == "recepcjonista":
