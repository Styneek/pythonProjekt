...
if __name__ == "__main__":
    dm = DataManager()
    guest_service = GuestService(dm)

    guest_service.add_guest("G001", "Jan", "Kowalski", "ABC123456", {"email": "jan.kowalski@example.com"})
    guest_service.add_guest("G002", "Anna", "Nowak", "XYZ987654", {"phone": "987654321"}, is_loyal_customer=True)

    print("\nPoczątkowi Goście:")
    guest_service.list_all_guests()

    guest_service.update_guest("G001", contact_info={"email": "jan.k@example.com", "phone": "111-222-333"}, is_vip=True)

    print("\nGoście po aktualizacji:")
    guest_service.list_all_guests()

    guest_service.add_stay_record_to_guest("G001", {"room_number": "101", "check_in": "2023-04-01", "check_out": "2023-04-05"})
    guest = guest_service.get_guest("G001")
    if guest: print(f"\nHistoria pobytów Gościa G001: {guest.stay_history}")

    guest_service.delete_guest("G002")
    print("\nGoście po usunięciu:")
    guest_service.list_all_guests() 