...
if __name__ == "__main__":
    dm = DataManager()
    room_service = RoomService(dm)

    room_service.add_room("101", 1, "single", 100.00, ["TV"])
    room_service.add_room("102", 1, "double", 150.00, ["Balcony", "AC"])
    room_service.add_room("201", 2, "suite", 300.00, ["Kitchen", "Jacuzzi"])

    print("\nPoczątkowe Pokoje:")
    room_service.list_all_rooms()

    room_service.update_room("101", price=110.00, amenities=["TV", "Wi-Fi"])
    room_service.update_room_status("102", "occupied")

    print("\nPokoje po aktualizacji:")
    room_service.list_all_rooms()

    print("\nDostępne Pokoje:")
    for room in room_service.find_available_rooms():
        print(room)

    room_service.delete_room("201")
    print("\nPokoje po usunięciu:")
    room_service.list_all_rooms()

    room_service.add_room("201", 2, "suite", 300.00, ["Kitchen", "Jacuzzi"])
    room_service.update_room_status("101", "cleaning")
    print("\nPokoje po kolejnych aktualizacjach:")
    room_service.list_all_rooms() 